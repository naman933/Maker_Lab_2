...
def normalize_date(date_str):
    ...
    for fmt in ['%d/%m/%Y', '%Y-%m-%d', '%d-%m-%Y', '%m/%d/%Y', '%B %d, %Y', '%b %d, %Y',
        ...
        for candidate_str in [s_clean, s]:
            cleaned = re.sub(r'(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),?\s*', '',
                             candidate_str, flags=re.IGNORECASE).strip()
            try:
...
def parse_pdfs_parallel(pdf_list, max_workers=4):
    scorecards = []
...
class handler(BaseHTTPRequestHandler):
    ...
    def _set_cors(self):
        ...
        self.send_header('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Keepalive-Token')
    ...
    def do_POST(self):
        try:
            ...
            if 'formdata_file' not in form:
                return self._json_response(400, {'detail': 'formdata_file is required'})
            if 'pdf_files' not in form:
                return self._json_response(400, {'detail': 'pdf_files is required'})

            formdata_item = form['formdata_file']
            form_bytes = formdata_item.file.read()
            form_name = formdata_item.filename or 'data.csv'

            candidates = parse_formdata(form_bytes, form_name)
            if not candidates:
                return self._json_response(400, {'detail': 'No valid candidates found in the uploaded file'})

            # pdf_files can be a single item or a list
            pdf_field = form['pdf_files']
            ...
            for item in pdf_items:
                ...
                pdfs.append({'bytes': pdf_bytes, 'filename': item.filename or 'unknown.pdf'})

            scorecards = parse_pdfs_parallel(pdfs)
            results = match_and_verify(candidates, scorecards)
            return self._json_response(200, results)

        except Exception as e:
            return self._json_response(500, {'detail': f'Internal server error: {str(e)}'})
